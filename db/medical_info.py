...
def convert_to_medical_card(table_medical_card: TableMedicalCard, table_medical_info: list[TableMedicalInfo],
                            table_vaccinations: list[TableVaccination]) -> MedicalCard:
...
def convert_to_table_medical_card_objects(medical_card: MedicalCard) -> tuple[
    TableMedicalCard, list[TableMedicalInfo], list[TableVaccination]]:
...
def find_medical_card_by_pet_id(pet_id: str) -> MedicalCard:
    ...
    table_medical_info = session.query(TableMedicalInfo).filter(
        or_(TableMedicalInfo.card_id == table_medical_card.id)).all()
    table_vaccinations = session.query(TableVaccination).filter(
        or_(TableVaccination.card_id == table_medical_card.id)).all()

    medical_card = convert_to_medical_card(table_medical_card, table_medical_info, table_vaccinations)

    return medical_card

def edit_medical_info(card_id: str) -> str:
