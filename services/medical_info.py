...
from db.medical_info import find_medical_card, add_medical_card, delete_medical_card, edit_medical_info, find_medical_card_by_pet_id
from db.pets import find_pet
from schemas.users import TokenAuth
from schemas.medical_info import MedicalCard, MedicalInfo, Vaccination, CreateMedicalCard, CreateMedicalInfo, \
    CreateVaccination
...
class MedicalInfoService:
    ...
    def edit_medical_info(self, card_id: str, token: TokenAuth) -> str:
        user = users_service._token_auth(token)
        card = find_medical_card(card_id)
        ...
        return edit_medical_info(card.id)
